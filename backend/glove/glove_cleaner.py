...
PATH_TO_GLOVE_FILE_DIR = "letterCache"
...
def splitWordsByFirstChar(words, saveToDisk=False):
    ...
    if saveToDisk:
        for first_char, words in splitDict.items():
            file_name = os.path.join(PATH_TO_GLOVE_FILE_DIR, first_char + ".txt")
...
def cleanWordEmbeddingData(path):
    ...
    cleaned_file_name = os.path.join(PATH_TO_GLOVE_FILE_DIR, "cleaned_" + file_name)
...
if __name__ == "__main__":
    g_path = os.path.join(PATH_TO_GLOVE_FILE_DIR, GLOVE_FILE_NAME)
